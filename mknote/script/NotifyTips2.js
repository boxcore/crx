 ...
 * 正在处理的任务状态是持续的提醒状态，
 * 瞬间操作为临时提醒，如将某某笔记加入到同步队列中等，仅显示几秒钟钟后，又显示当前的操作
 */
...
// if (!webkitNotifications.createHTMLNotification) {
    ...
    var NotifyTips = function() {
        ...
        var create = function(data) {
            var notification = new Notification(data.data.content, {icon:'images/icons/48x48.png',});
    ...
    var ReadyErrorNotify = (function() {
        ...
        return {
            show: function(key, timer) {
                ...
                notification = new Notification(errorContent);
                notification.addEventListener('close', function(e) {
                    notification = null;
                });
                notification.show();
